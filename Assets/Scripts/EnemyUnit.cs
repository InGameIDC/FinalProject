...
{
    public enum enemyStatus { dead, siege, moving, idle, attacking, moveAndAttack, rotating, moveAndRotate, moveAndAttackAndRotate, rotateAndAttack, onHill};
    ...
    public enemyStatus _status;                    // The hero order status (CURRECTLY NOT IN USE, MIGHT BE REMOVED)
    ...
    private void Awake()
    {
        ...
        initScanner();
        initHeroHealth();
        _hill = GameObject.FindGameObjectWithTag("Hill");
    }
    ...
    private void manageTargetAddDuringIdle(GameObject target)
    {
        ...
        if (_skill.isTargetAttackable(target)) // If the target is attackable, the hero would start to attack
        {
            ...
            if (target == _targetObj || _targetObj == null) // if I dont have a target, or its my _targetObj, make the enemy that entered the target
            {
                ...
                _targetToAttack = target;
                Debug.Log("prepare to attack " + _targetToAttack);
                prepareToAttack(); // how to attack, when, etc.
            }
            /*
    ...
    {
        Debug.Log("rotating? " + _movement.IsObjRotating());
